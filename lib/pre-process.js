/*
This code aggregate the data from five files into one. 
The output data structure is: {"word": string, "auto": string, "overrides": string, "rank": double}
*/
exports.read = function () {
	...
	fs.readFile(dictENfilepath, {encoding: 'utf8'}, function (err, content) {
		...
		for (var i = 1; i < line.length; i++) {
			...
			//initially, I assigned every records a rank value log(2)
			//So I can avoid looping the popularity data which value is smaller than 2
			//Almost half of the records have the value less than 2, so this will save a lot of time
			var num = Math.log(2);
			var word = record[2];
				...
				fs.readFile(overrideESfilepath, {encoding: 'utf8'}, function (err, content) {
					...
					fs.readFile(popularityfilepath, {encoding: 'utf8'}, function (err, content) {
						...
						for (var i = 1; i < line.length ;i++) {
							...
							//If the value less than the threshod, skip it
